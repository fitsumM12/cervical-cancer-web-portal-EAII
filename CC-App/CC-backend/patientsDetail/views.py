...
def monthly_patient_count(request, year):
    try:
        ...
        monthly_data = [
            {"month": data['month'].strftime('%b'), "year": year, "patient_count": data['patient_count']}
            for data in patient_data
        ]
        months = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"]
        response_data = [
            {"month": month, "year": year, "patient_count": next((item['patient_count'] for item in monthly_data if item['month'] == month), 0)}
            for month in months
        ]
        serializer = MonthlyPatientCountSerializer(response_data, many=True)
        return Response(serializer.data)
